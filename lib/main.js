...
export async function main(
  ...
  if (permissions) {
    const sep = permissions.includes(",") ? "," : "\n";
    const permissionSpecs = permissions.split(sep);
    for (spec of permissionSpecs) {
      const [key, value] = spec.split(":")
      parsedPermissions[key.trim()] = value.trim();
    }
  }
  ...
  if (parsedRepositoryNames) {
    ({ authentication, installationId, appSlug } = await pRetry(() => getTokenFromRepository(request, auth, parsedOwner, parsedRepositoryNames, parsedPermissions), {
    ...
    ({ authentication, installationId, appSlug } = await pRetry(() => getTokenFromOwner(request, auth, parsedOwner, parsedPermissions), {
      onFailedAttempt: (error) => {
        core.info(
...
async function getTokenFromOwner(request, auth, parsedOwner, parsedPermissions) {
  ...
  if (Object.keys(parsedPermissions).length > 0) {
    authConfig["permissions"] = parsedPermissions;
  }

  // Get token for for all repositories of the given installation
  const authentication = await auth(authConfig);
...
async function getTokenFromRepository(request, auth, parsedOwner, parsedRepositoryNames, parsedPermissions) {
